...
public class SpacesController : ControllerBase
{
    ...
    public async Task<ActionResult<Space>> GetSpace(Guid id)
    {
        ...
        if (!await _context.IsParticipant(_userId, id)) return Unauthorized();
    ...
    public async Task<IActionResult> DeleteSpace(Guid id)
    {
        ...
        if (!await _context.IsHost(_userId, id)) return Unauthorized();
    ...
    public async Task<ActionResult<SpaceAccessCode>> OpenSpace(Guid id)
    {
        ...
        if (space.AccessCode != null) _context.SpaceAccessCode.Remove(space.AccessCode);

        var uniqueCode = new Random().Next(0, 1000000).ToString("D6");
    ...
    public async Task<ActionResult<Space>> JoinSpace(SpaceAccessCodeRequest accessCode)
    {
        ...
        if (space == null || space.AccessCode!.ExpiresAt < DateTime.Now) return NotFound();

        if (await _context.IsParticipant(_userId, space.Id)) return space;
