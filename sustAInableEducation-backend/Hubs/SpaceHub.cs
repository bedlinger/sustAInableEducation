...
public class SpaceHub : Hub
{
    ...
    {
        if (!await _context.IsParticipant(_userId, _spaceId)) throw new HubException("Unauthorized");
    ...
    {
        if (!await _context.IsHost(_userId, _spaceId)) throw new HubException("Unauthorized");

        var space = (await _context.SpaceWithStory.FirstOrDefaultAsync(e => e.Id == _spaceId))!;
        var story = space.Story;
        if (story.Result != null) throw new HubException("Story is already complete");
    ...
    public async Task StartVoting()
    {
        ...
        var space = (await _context.SpaceWithStory.FirstOrDefaultAsync(e => e.Id == _spaceId))!;
        if (space.Story.Result != null) throw new HubException("Story is complete");
        ...
        if (part.IsVotingActive) throw new HubException("Voting is already active");

        _context.SpaceParticipant.Where(p => p.SpaceId == _spaceId)
            .ExecuteUpdate(p => p
                .SetProperty(x => x.VoteImpact, x => null));
        foreach (var choice in part.Choices) choice.NumberVotes = 0;
    ...
    public async Task Vote(uint number)
    {
        var part = (await _context.SpaceWithStory.FirstOrDefaultAsync(e => e.Id == _spaceId))!.Story.Parts
            .LastOrDefault();
        if (part == null) throw new HubException("No part");

        if (!part.IsVotingActive) throw new HubException("Voting is not active");

        var participant = (await _context.SpaceParticipant.FindAsync(_spaceId, _userId))!;
        if (participant.VoteImpact != null) throw new HubException("Already voted");
    ...
    public async Task SetChoice(uint number)
    {
        ...
        if (part.ChosenNumber != null) throw new HubException("Already chosen");

        var choice = part.Choices.First(c => c.Number == number);
        if (choice == null) throw new HubException("Invalid choice");
